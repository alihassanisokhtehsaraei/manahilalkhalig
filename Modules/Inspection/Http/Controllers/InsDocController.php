    ...
    private function uploadFiles($files, $description, $inputName, $folderName, $orderId, $request) {
        if (!empty($files)) {
            foreach ($files as $file) {
                ...
                $doc->orderID = $orderId;
                $doc->status = (Auth()->user()->level == 'supervisor' || Auth()->user()->level == 'manager') ? 2 : 0;
                $doc->reviewerID = (Auth()->user()->level == 'supervisor' || Auth()->user()->level == 'manager') ? Auth()->user()->id : null;
                $doc->reviewTime = (Auth()->user()->level == 'supervisor' || Auth()->user()->level == 'manager') ? date('Y-m-d H:i:s') : null;
    ...
    public function storeother(Request $request, $orderId)
    {
        ...
        if ($request->hasFile('other')) {
            ...
            foreach ($files as $file) {
                ...
                $doc->userID = Auth()->user()->id;
                $doc->orderID = $orderId;
                $doc->status = (Auth()->user()->level == 'supervisor' or Auth()->user()->level == 'manager') ? 2 : 0;
                $doc->reviewerID =(Auth()->user()->level == 'supervisor' or Auth()->user()->level == 'manager') ? Auth()->user()->id : null;
                $doc->reviewTime =(Auth()->user()->level == 'supervisor' or Auth()->user()->level == 'manager') ? date('Y-m-d H:i:s') : null;
                $doc->url = "ipms/inspectionFiles/{$folderName}/{$newName}";
                $doc->ip = $request->ip();
                $doc->save();
            }
