                        ...
                        @foreach($order?->nonReleaseDocuments as $index => $doc)
                            <tr>
                                ...
                                <td>
                                    ...
                                    <a href="{{route('nrdocs.edit',['order'=>$order,'nonReleaseDocument'=>$doc])}}" class="btn btn-warning btn-sm">Open</a>
                               @if($doc->status == "1")
                                    <button type="button" class="btn btn-danger btn-sm" data-bs-toggle="modal" data-bs-target="#deleteModal" data-id="{{ $doc->id }}">
                                        Delete
                                    </button>
                                    @endif
                                    <a href="{{route('nrdocs.showUpload',['order'=>$order,'nonReleaseDocument'=>$doc])}}" class="btn btn-secondary btn-sm">Uploads</a>

                                    @if($doc->status == "2") <a href="{{URL::signedRoute('words.nrd',$doc->id)}}" class="btn btn-primary btn-sm">Print</a> @endif
                                </td>
                            </tr>
                        @endforeach
                        </tbody>
